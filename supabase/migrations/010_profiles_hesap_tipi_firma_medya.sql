-- AURIX 010 — Hesap tipi (normal / firma) + firma medya alanları
-- Production uyumlu: var olmayan kolonlara (is_seed, owner_id) sabit referans YOK.
-- apply_008 ile aynı yaklaşım: information_schema kontrolü + dinamik policy.
...
-- Panel SELECT grant: yalnızca gerçekten var olan sütunlar
DO $$
DECLARE
    wanted text[] := ARRAY[
...
-- Mevcut firma sahiplerini geriye dönük eşle (yalnızca user_id varsa)
DO $$
BEGIN
...
-- handle_new_user_profile: hesap_tipi her zaman normal
...
-- ============================================================
-- 5. Teklif INSERT politikası — production şemasına göre dinamik
--    Production (008): firmalar.user_id var; is_seed / owner_id YOK olabilir.
-- ============================================================
DO $$
DECLARE
...
-- Temizlik: yardımcı fonksiyon (opsiyonel; bırakılabilir)
-- DROP FUNCTION IF EXISTS public._aurix_col_exists(text, text);
