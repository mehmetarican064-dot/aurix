...
(function () {
    ...
    function firmaKartHtml(firma, opts) {
        ...
        return '<article class="firma-kart' + premiumCls + '" data-id="' + firma.id + '">' +
            ...
            '</div>' +
            '<div class="firma-kart__puan-alan">' +
            (firma.puan ? '<span class="firma-kart__puan">' + yildizGoster(firma.puan) + '</span>' : '<span class="firma-kart__puan firma-kart__puan--bos">&nbsp;</span>') +
            '</div>' +
    ...
    function renderFirmaBolumleri() {
        ...
        AURIX_DATA.FIRMA_BOLUMLERI.forEach(function (bolum) {
            ...
            el.innerHTML = list.length
                ? list.map(function (f) { return firmaKartHtml(f); }).join('')
    ...
    function renderVitrin(sayfaSifirla) {
        ...
        if (sayac) {
            if (!list.length) {
                sayac.textContent = '0 firma listeleniyor';
    ...
    function renderKategoriGruplari() {
        ...
        if (anaListe && anaListe.length) {
            ...
            el.innerHTML = anaListe.map(function (item) {
                var gorselIcerik = item.gorsel
                    ? '<img src="' + item.gorsel + '" alt="" class="esnaf-kategori-kutu__img" width="160" height="120" loading="lazy" decoding="async">'
                    : '<span class="esnaf-kategori-kutu__placeholder" aria-hidden="true"></span>';
                return '<button type="button" class="esnaf-kategori-kutu" data-kat="' + item.id + '">' +
                    '<div class="esnaf-kategori-kutu__gorsel">' + gorselIcerik + '</div>' +
                    '<span class="esnaf-kategori-kutu__ad">' + item.ad + '</span></button>';
            }).join('');
        } else {
    ...
    function renderNedenAurix() {
        ...
        el.innerHTML = AURIX_DATA.NEDEN_AURIX.map(function (item) {
            return '<article class="neden-kart">' +
                '<div class="neden-kart__ikon">' + item.ikon + '</div>' +
    ...
    window.Aurix = { sayfaGoster: sayfaGoster, modalKapat: modalKapat, demoSifirla: demoVeriSifirla };
